...
while True:
    ...
    if(str(label_map[int(classes[i])]) == 'BlueHat' or str(label_map[int(classes[i])]) == 'RedHat'):
        ...
        # Convert object name to equivalent integer value.
        if(str(label_map[int(classes[i])]) == 'BlueHat'):
            val = 1
        elif (str(label_map[int(classes[i])]) == 'RedHat'):
            val = 2
        
        while(flag):
            if (count == 0):
                ser.write(str(val).encode('utf-8')) #Encode the val to enable it to be sent and read by Arduino serial
                print(str(val).encode('utf-8'))
                print(int(confidence[i]*1000))
            time.sleep(2)
            if (count == 7 ):
                print(flag)
                flag = False
